...
void Position()
{
    readPosition(node.ID % 16, node.ID % 8, 1);
...
void head_Record_Member()
{
    for (int j = 0; j < 8; j++)
    {
        ...
        node.record_member[j] = node.ID + j;
    }
...
{
    node.backup = node.ID + 1; // 鎵�鍦ㄧ皣澶囦唤绨囬
